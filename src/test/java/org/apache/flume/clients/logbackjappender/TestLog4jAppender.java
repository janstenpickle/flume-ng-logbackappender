...
public class TestLog4jAppender{
  ...
  public void testLog4jAppender() throws IOException {
    ...
    for(int count = 0; count <= 1000; count++){
      ...
      Assert.assertNotNull(hdrs.get(LogbackAvroHeaders.TIMESTAMP.toString()));

      Assert.assertEquals(Level.toLevel(level),
          Level.toLevel(Integer.valueOf(hdrs.get(LogbackAvroHeaders.LOG_LEVEL
              .toString()))
          ));

      Assert.assertEquals(logger.getName(),
          hdrs.get(LogbackAvroHeaders.LOGGER_NAME.toString()));

      Assert.assertEquals("UTF8",
          hdrs.get(LogbackAvroHeaders.MESSAGE_ENCODING.toString()));
      transaction.commit();
      transaction.close();
    }
  ...
  public void testLayout() throws IOException {
    ...
    for(int count = 0; count <= 100; count++){
      ...
      Assert.assertEquals(Level.toLevel(level),
          Level.toLevel(Integer.parseInt(hdrs.get(LogbackAvroHeaders.LOG_LEVEL
              .toString()))));
